    ...
    try:
        async with async_playwright() as p:
            ...
            try:
                ...
                yield f"⚠️ Failed to launch {browser_type}, falling back to chromium"
            ...
            try:
                ...
                # 🧹 Check and delete rows if filled (if user manually created one)
                async for msg in ts.delete_all_rows():
                    yield msg

                # 🌀 Re-open the same timesheet now that it's clean
                await ts.goto_timesheets()
                await ts.open_latest_timesheet()
            ...
            if await ts.is_submission_successful():
                ...
                yield "⚠️ Submission may need manual check."
...
#                     user_mode = "create"
#                 except Exception as e:
#                     print(f"❌ Timesheet creation failed: {str(e)}")
#                     yield f"❌ Timesheet creation failed: {str(e)}"
#                     return
...
#                     await ts.fill_multiple_timesheets(params)
#             except Exception as e:
#                 print(f"❌ Error while filling timesheet: {str(e)}")
#                 yield f"❌ Error while filling timesheet: {str(e)}"
#                 return
...
#                 yield "Timesheet submitted successfully."
#             else:
#                 yield "⚠️ Submission may needs manual check."
